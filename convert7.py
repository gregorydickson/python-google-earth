...
def main():
  ...
  # It uses a format for the fields and delimiter defined in format.txt
  file = open('format.txt')
  order = file.readline().split(',')
  fileDelimiter = file.readline()
  csvreader = csv.DictReader(open('sites.csv'),order,delimiter = fileDelimiter)
